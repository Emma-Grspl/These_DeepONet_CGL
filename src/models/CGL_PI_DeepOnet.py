...
class CGL_PI_DeepONet(nn.Module):
    def __init__(self, cfg):
        ...
        eq_p = cfg.physics['equation_params'] # NOUVEAU
        ...
        # NOUVEAU : On normalise alpha, beta, mu, V
        ...
        # 3. BRANCH NET
        # Entrées : [alpha, beta, mu, V, A, w0, x0, k, type] -> 9 paramètres
    ...
    def normalize_linear(self, x, x_min, x_max):
        ...
        denom = x_max - x_min
        # Si x_max == x_min (paramètre fixe), on évite la division par zéro
        denom = torch.where(denom == 0, torch.tensor(1.0, device=x.device), denom)
    ...
    def forward(self, params, coords):
        # params: [alpha, beta, mu, V, A, w0, x0, k, type] (Batch x 9)
        ...
        # Concaténation de TOUS les paramètres normalisés
