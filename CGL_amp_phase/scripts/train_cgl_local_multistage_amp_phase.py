...
def rollout_multistage_models(models, time_blocks, trajectory, cfg_dict, device):
    ...
    with torch.no_grad():
        for step in range(n_steps):
            model = select_stage_model(models, time_blocks, float(trajectory["t_values"][step]))
            branch_vec = build_branch_features(cfg_dict, pred_sensor[:, step], dt_value, params)
...
def plot_rollout_curve(rollout, output_path):
    ...
    plt.axvline(2.0, color="black", linestyle=":", linewidth=1.0)
    plt.axvline(3.0, color="black", linestyle=":", linewidth=1.0)
    plt.axvline(4.0, color="black", linestyle=":", linewidth=1.0)
    plt.xlabel("Temps t")
...
def main():
    ...
    for stage_idx, (t_start, t_end) in enumerate(time_blocks):
        ...
        train_one_stage(model, optimizer, trajectory, cfg_dict, train_idx, valid_idx, stage_dir, device)
