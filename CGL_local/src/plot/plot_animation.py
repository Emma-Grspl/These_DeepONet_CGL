...
def animate_cgl_solution(
    ...
    # Gestion souple Config (Dict vs Objet)
    ...
    # Lignes
    line_true_mod, = ax_mod.plot([], [], color=classical_color, lw=2, label='Exact')
    line_pred_mod, = ax_mod.plot([], [], color=model_color, linestyle='--', lw=2, label='PI-DeepONet')
    ...
    def update(frame_idx):
        ...
        # Mise à jour du titre (On garde les params statiques et on change le temps)
        title.set_text(f"{param_str}\nt = {current_t:.2f}s")
    ...
    anim = animation.FuncAnimation(fig, update, frames=frames, interval=50, blit=False) # Blit=False est souvent plus stable
    ...
    if save_path.endswith('.gif'):
        ...
        try:
            ...
            print(f"⚠️ Erreur ffmpeg, fallback sur .gif")
