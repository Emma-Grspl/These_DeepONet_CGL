...
def format_case_label(params_dict):
    return (
        rf"$\alpha$={params_dict['alpha']:.3f}, "
        rf"$\mu$={params_dict['mu']:.3f}, "
        rf"$\beta$={params_dict['beta']:.3f}"
    )
...
def plot_error_heatmap(model, cfg_dict, params_dict, t_curr, output_path, device):
    ...
    plt.title(f"Heatmap de l'erreur absolue | {format_case_label(params_dict)}")
...
def plot_mean_error_vs_time(
    ...
    title = f"Évolution temporelle de l'erreur moyenne sur {n_cases} cas"
    if title_suffix:
        title = f"{title}\n{title_suffix}"
    plt.title(title)
    plt.grid(alpha=0.25)
